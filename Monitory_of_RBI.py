...
Patched: Light/Dark theme toggle, Plotly theme adapter, yfinance live-market tab integration.
"""
...
try:
    ...
    # If auth not present, create no-op to avoid crashes; but it's recommended to include auth.py
    def require_login():
        return
    def logout_button():
        return

# yfinance live module
try:
    from rbi_yfinance_live_module import (
        render_live_market_tab,
        render_live_sidebar_panel,
    )
except Exception:
    # Stubs if module missing
    def render_live_market_tab(*args, **kwargs):
        st.warning("rbi_yfinance_live_module.py not found. Place it in the same folder.")
...
# Provide theme toggle in sidebar
with st.sidebar:
    # logout (left) + theme toggle (right)
    logout_button()
...
.header-title { color: white !important; font-size: 24px; font-weight:700; text-align:center; font-family:'Merriweather', serif; }
.rbi-card { padding:16px; border:2px solid #00336633; border-radius:10px; background-color:#F2F7FF; box-shadow:0px 2px 6px #00336622; }
.stTabs [role="tablist"] button { font-size:15px; font-weight:600; background-color:#E8EEF7; color:#003366 !important; border-radius:5px; margin-right:4px; padding:6px 14px; }
...
.header-title { color:#58a6ff !important; font-size:24px; font-weight:700; text-align:center; font-family:'Merriweather', serif; }
.rbi-card { padding:16px; border:1px solid #30363d; border-radius:10px; background-color:#1f242b; box-shadow:0px 2px 6px #00000066; color:#c9d1d9 !important; }
.stTabs [role="tablist"] button { font-size:15px; font-weight:600; background-color:#0f1720; color:#c9d1d9 !important; border-radius:5px; margin-right:4px; padding:6px 14px; }
...
# --------------------
# SIDEBAR CONTROLS (same UI as before)
# --------------------
with st.sidebar:
    ...
    use_fred = st.checkbox("Use FRED for US data (CPI, Fed, Oil, Gold)", value=True)
    st.markdown("---")
...
# --------------------
# HELPERS & Fetchers (same as before, using @st.cache_data where appropriate)
# --------------------
def safe_num(x):
    try:
        return float(x)
...
def scrape_rbi_press_releases(max_items=10):
    ...
    try:
        ...
        for a in links:
            ...
            href = a["href"]
            if any(kw in title.lower() for kw in ["monetary policy", "policy statement", "repo rate", "mpc"]):
...
def fetch_forex_inr(symbols=("USD", "EUR", "GBP", "JPY"), years=5):
    try:
        ...
        url = f"https://api.exchangerate.host/timeseries?start_date={start}&end_date={end}&base=INR&symbols={','.join(symbols)}"
        r = requests.get(url, timeout=12)
        r.raise_for_status()
        data = r.json()
...
def sample_econ_calendar():
    data = [
        {"Event": "RBI MPC Meeting", "Date": "2025-02-08", "Type": "Policy", "Importance": "High"},
        {"Event": "India CPI Release", "Date": "2025-01-15", "Type": "Inflation", "Importance": "High"},
        {"Event": "India GDP Data", "Date": "2025-03-01", "Type": "Growth", "Importance": "Medium"},
        {"Event": "US Fed FOMC Meeting", "Date": "2025-01-29", "Type": "Policy", "Importance": "High"},
    ]
    return pd.DataFrame(data)

def compute_mpi_msi_and_insights(wb_infl_in, m3_in, rbi_repo, liq_monthly, repo_monthly, w_infl, w_rate, w_liq, w_vol):
    ...
    for v in raw_vals:
        if np.isnan(v):
            ...
            norm_vals.append(np.tanh(v / 10))
    msi = float(np.mean(norm_vals) * 50 + 50)
...
m3_in = fetch_worldbank_indicator("IN", "FM.LBL.MQMY.GD.ZS")
...
# --------------------
# TABS (including Live Market Data tab)
# --------------------
...
with overview_tab:
    ...
    with col1:
        ...
        if not wb_infl_in.empty:
            fig = px.line(wb_infl_in, x="Date", y="Value", title="India: Inflation (annual %)", template=get_plotly_template())
    ...
    with col2:
        ...
        if not wb_infl_us.empty:
            fig = px.line(wb_infl_us, x="Date", y="Value", title="US: Inflation (annual %)", template=get_plotly_template())
            st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    if not fred_cpi_us.empty:
        ...
        st.plotly_chart(px.line(fred_cpi_us.reset_index(), x="Date", y="Value", title="US CPI (FRED, monthly)", template=get_plotly_template()), use_container_width=True)
...
with rbi_live_tab:
    ...
    if not rbi_rates_live.empty:
        ...
        with c2:
            core_rates = rbi_rates_live[rbi_rates_live["Indicator"].isin(["Repo Rate", "MSF", "SDF", "Bank Rate"])]
            if not core_rates.empty:
                fig_p = px.bar(core_rates, x="Indicator", y="Value", title="RBI Policy Corridor — Repo, MSF, SDF, Bank Rate", template=get_plotly_template(), text_auto=".2f")
    ...
    if not laf_live.empty:
        ...
        with c4:
            fig_laf = px.bar(laf_live, x="Field", y="Value", title="RBI Daily Liquidity — Net LAF, SDF, MSF, Call Money", template=get_plotly_template(), text_auto=".0f")
    ...
    if not m3_weekly_live.empty:
        ...
        with c6:
            fig_m3 = px.bar(m3_weekly_live, x="Indicator", y="Value", title="Weekly Monetary Aggregates — M3, RM, Currency, Deposits", template=get_plotly_template(), text_auto=".0f")
...
with liq_repo_tab:
    ...
    if uploaded_liq is None:
        if not m3_in.empty:
            ...
            st.plotly_chart(px.line(m3_in, x="Date", y="Value", title="India M3 (World Bank indicator)", template=get_plotly_template()), use_container_width=True)
        else:
            st.info("No auto M3 data available — you can upload a liquidity CSV to override.")
    else:
        try:
            ...
            st.plotly_chart(px.line(plot_liq.reset_index(), x="Date", y=plot_liq.columns, title="Uploaded Liquidity Data (Monthly)", template=get_plotly_template()).update_layout(hovermode="x unified"), use_container_width=True)
    ...
    if uploaded_repo is None:
        if not rbi_repo.empty:
            ...
            with colm1:
                ...
                st.markdown(f"<p><b>{latest_repo:.2f} %</b></p>", unsafe_allow_html=True)
                st.markdown("</div>", unsafe_allow_html=True)
            with colm2:
                st.write("")
            st.plotly_chart(px.line(rbi_repo.reset_index().rename(columns={"index": "Date"}), x="Date", y="RepoRate", title="RBI Repo Rate (scraped)", template=get_plotly_template()).update_xaxes(title="Date"), use_container_width=True)
        else:
            st.info("RBI repo rate not found via scraping — upload a Repo CSV for full analysis.")
    else:
        try:
            ...
            st.plotly_chart(px.line(plot_repo.reset_index(), x="Date", y=plot_repo.columns, title="Uploaded Repo Rate (Monthly)", template=get_plotly_template()).update_layout(hovermode="x unified"), use_container_width=True)
        except Exception as e:
            st.warning(f"Failed to read Repo CSV: {e}")

    st.markdown("<div class='rbi-divider'></div>", unsafe_allow_html=True)

    # Merged series + correlations + heatmap (same as before)
    merged = pd.DataFrame()
    if 'liq_monthly' in locals() and not liq_monthly.empty:
        merged = liq_monthly.copy()
    if not repo_monthly.empty:
        merged = repo_monthly.copy() if merged.empty else merged.join(repo_monthly, how="outer")
    if not fred_cpi_us.empty:
        merged = fred_cpi_us.rename(columns={"Value": "US_CPI"}).copy() if merged.empty else merged.join(fred_cpi_us.rename(columns={"Value": "US_CPI"}), how="left")
    if not wb_infl_in.empty:
        try:
    ...
    if not merged.empty:
        ...
        fig.update_layout(title="Combined Liquidity + Repo + CPI (monthly)", template=get_plotly_template(), hovermode="x unified")
        st.plotly_chart(fig, use_container_width=True)

        st.markdown("#### Correlation Matrix (merged series)")
        corr = merged.corr()
        try:
            fig_corr = px.imshow(corr, text_auto=True, aspect="auto", color_continuous_scale="RdBu_r", title="Correlation Matrix", template=get_plotly_template())
        ...
        try:
            ...
            fig_heat = px.imshow(z.T, aspect="auto", color_continuous_scale="RdBu_r", labels={"x": "Time", "y": "Variable", "color": "Z-score"}, title="Liquidity Stress Heatmap", template=get_plotly_template())
            st.plotly_chart(fig_heat, use_container_width=True)
        except Exception as e:
            st.warning(f"Could not compute stress heatmap: {e}")
    else:
        st.info("No merged monthly series yet — upload liquidity and/or repo CSVs or enable FRED.")

    st.markdown("<div class='rbi-divider'></div>", unsafe_allow_html=True)

    # Yield curve visualizer, bank credit, MPC tracker, etc. (unchanged)
    st.markdown("#### Yield Curve Visualizer (Upload Government Bond Yields)")
    yc_file = st.file_uploader("Upload Yield Curve CSV (Date + maturities like 3M, 1Y, 2Y, 5Y, 10Y)", type="csv", key="yc_upload")
    if yc_file is not None:
        try:
            ...
            mat_cols = st.multiselect("Select maturity columns (yields)", [c for c in cols if c != date_col_y], default=[c for c in cols if c != date_col_y])
            yc_df_raw[date_col_y] = pd.to_datetime(yc_df_raw[date_col_y], errors="coerce")
            ...
            plot_df = pd.DataFrame({"Maturity": mat_cols, "Yield": last_row.values})
            fig_yc = px.line(plot_df, x="Maturity", y="Yield", markers=True, title="Latest Yield Curve", template=get_plotly_template())
    ...
    credit_file = st.file_uploader("Upload Bank Credit CSV (Date, TotalCredit or sectoral columns)", type="csv", key="credit_upload")
    if credit_file is not None:
        try:
            cr_df = pd.read_csv(credit_file)
            cols = cr_df.columns.tolist()
            date_col_cr = st.selectbox("Select Date column", cols, key="credit_date_col")
            cr_df[date_col_cr] = pd.to_datetime(cr_df[date_col_cr], errors="coerce")
            ...
            fig_cr = px.line(cr_df, x=date_col_cr, y=val_cols, title="Bank Credit Over Time", template=get_plotly_template())
            st.plotly_chart(fig_cr, use_container_width=True)
            if "TotalCredit" in cr_df.columns:
                cr_df["CreditGrowthYoY"] = (cr_df["TotalCredit"].pct_change(periods=12) * 100)
                fig_gry = px.line(cr_df, x=date_col_cr, y="CreditGrowthYoY", title="YoY Bank Credit Growth (%)", template=get_plotly_template())
    ...
    mpc_file = st.file_uploader("Upload MPC vote CSV (MeetingDate, HikeVotes, HoldVotes, CutVotes)", type="csv", key="mpc_upload")
    if mpc_file is not None:
        try:
            ...
            if date_col_mpc is None:
                st.warning("No date column detected — please include a column like 'MeetingDate'.")
            else:
                mpc_df = mpc_df.dropna(subset=[date_col_mpc])
                mpc_df = mpc_df.sort_values(date_col_mpc)
                mpc_melt = mpc_df.melt(id_vars=[date_col_mpc], value_vars=[c for c in mpc_df.columns if "Votes" in c], var_name="VoteType", value_name="Count")
                fig_mpc = px.bar(mpc_melt, x=date_col_mpc, y="Count", color="VoteType", barmode="stack", title="MPC Voting Pattern Over Time", template=get_plotly_template())
...
with global_tab:
    ...
    country_map = {
        ...
        "Euro Area": ("XZ", "FP.CPI.TOTL.ZG"),
        "Japan": ("JP", "FP.CPI.TOTL.ZG"),
        "China": ("CN", "FP.CPI.TOTL.ZG"),
    }
    sel_countries = st.multiselect("Select countries to compare (World Bank inflation)", options=list(country_map.keys()), default=["India", "United States"])
    global_df = pd.DataFrame()
    for cname in sel_countries:
        ccode, ind = country_map[cname]
        df_c = fetch_worldbank_indicator(ccode, ind)
        if not df_c.empty:
            df_c = df_c[(df_c["Date"].dt.year >= start_year) & (df_c["Date"].dt.year <= end_year)]
            df_c["Country"] = cname
            global_df = pd.concat([global_df, df_c], ignore_index=True)
    if not global_df.empty:
        fig_global = px.line(global_df, x="Date", y="Value", color="Country", title="Global Inflation Comparison (World Bank)", template=get_plotly_template())
        st.plotly_chart(fig_global, use_container_width=True)
    else:
        st.info("No global data fetched — try a different selection of countries or years.")

    st.markdown("<div class='rbi-divider'></div>", unsafe_allow_html=True)
    st.subheader("Economic Calendar (Monetary Policy & Macro Events)")
    cal_df = sample_econ_calendar()
    cal_json = st.text_area("Edit or add events (JSON list with Event, Date, Type, Importance):", value=json.dumps(cal_df.to_dict(orient="records"), indent=2), height=200)
    ...
    try:
        ...
        fig_cal = px.timeline(cal_parsed, x_start="Date", x_end="Date", y="Event", color="Importance", title="Timeline of Key Events", template=get_plotly_template())
...
with fx_com_tab:
    ...
    if not fx_df.empty:
        fx_choice = st.multiselect("Select FX pairs (value = foreign currency per INR)", options=list(fx_df.columns), default=["USD", "EUR"])
        if fx_choice:
            fig_fx = px.line(fx_df.reset_index(), x="Date", y=fx_choice, title="INR FX Rates (exchangerate.host)", template=get_plotly_template())
    ...
    if not oil_fred.empty or not gold_fred.empty:
        ...
        fig_com = px.line(com_df.reset_index(), x="Date", y=com_df.columns, title="Brent Oil & Gold Prices (FRED)", template=get_plotly_template())
        st.plotly_chart(fig_com, use_container_width=True)
    else:
        st.info("Commodity data not available (FRED disabled or API issue).")

# --------------------
# LIVE MARKET DATA TAB (calls yfinance module)
# --------------------
...
with forecast_tab:
    ...
    if not combined_annual.empty:
        ...
        fig = go.Figure()
        if not wb_infl_in.empty:
            fig.add_trace(go.Scatter(x=wb_infl_in["Date"], y=vol["IN_infl"], name="India volatility"))
        if not wb_infl_us.empty:
            fig.add_trace(go.Scatter(x=wb_infl_us["Date"], y=vol["US_infl"], name="US volatility"))
        fig.update_layout(title="Rolling Volatility (Annual Inflation, 3-year window)", template=get_plotly_template())
        st.plotly_chart(fig, use_container_width=True)
    else:
    ...
    if not wb_infl_in.empty:
        try:
            ...
            fig.update_layout(title="ARIMA Forecast — India Inflation (annual)", template=get_plotly_template())
            st.plotly_chart(fig, use_container_width=True)
        except Exception as e:
    ...
    mpi, msi, infl_trend, rate_text, stress_text, missing = compute_mpi_msi_and_insights(
        wb_infl_in, m3_in, rbi_repo, liq_monthly, repo_monthly, w_infl, w_rate, w_liq, w_vol
    )
    ...
    with col_mpi:
        ...
        st.markdown(f"<p><b>{round(mpi, 4)}</b></p>", unsafe_allow_html=True)
        st.markdown(f"<p>Weights — Inflation: {w_infl:.2f}, Rate: {w_rate:.2f}, Liquidity: {w_liq:.2f}, Volatility: {w_vol:.2f}</p>", unsafe_allow_html=True)
        if missing >= 3:
            st.markdown("<p><i>Note: Many inputs are missing, MPI may be unreliable. Upload Repo & Liquidity CSVs for better accuracy.</i></p>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

    with col_msi:
        fig_g = go.Figure(go.Indicator(mode="gauge+number", value=msi, title={"text": "Monetary Stress Index (MSI)"}, gauge={"axis": {"range": [0, 100]}, "steps": [{"range": [0, 40], "color": "green"}, {"range": [40, 70], "color": "yellow"}, {"range": [70, 100], "color": "red"}]}))
...
with report_tab:
    ...
    if not rbi_news.empty:
        ...
        st.info("Could not fetch RBI press releases (or no recent monetary policy items detected).")

    st.markdown("<div class='rbi-divider'></div>", unsafe_allow_html=True)

    mpi_pdf, msi_pdf, infl_trend_pdf, rate_text_pdf, stress_text_pdf, _ = compute_mpi_msi_and_insights(
        wb_infl_in, m3_in, rbi_repo, liq_monthly, repo_monthly, w_infl, w_rate, w_liq, w_vol
    )
    ...
    def save_figures_for_pdf(template_name):
        ...
        try:
            if not wb_infl_in.empty:
                ...
                fig = px.line(wb_infl_in, x="Date", y="Value", title="India Inflation", template=template_name)
                try:
                    fig.write_image(f)
                    out.append(f)
                except Exception:
            ...
            if not wb_infl_us.empty:
                ...
                fig = px.line(wb_infl_us, x="Date", y="Value", title="USA Inflation", template=template_name)
                try:
                    fig.write_image(f)
                    out.append(f)
                except Exception:
            ...
            if 'repo_monthly' in locals() and not repo_monthly.empty:
                f = "tmp_imgs/repo.png"
                fig = px.line(repo_monthly.reset_index(), x="Date", y=repo_monthly.columns[0], title="Repo Rate (Monthly)", template=template_name)
                try:
                    fig.write_image(f)
                    out.append(f)
                except Exception:
            ...
            if not fed_bs.empty:
                ...
                fig = px.line(fed_bs.reset_index(), x="Date", y="Value", title="Fed Balance Sheet", template=template_name)
                try:
                    fig.write_image(f)
                    out.append(f)
                except Exception:
    ...
    st.caption("End of RBI-style monetary policy dashboard. If any auto-fetch failed, upload CSVs or check network/API keys.")

# optionally render small live snapshot in sidebar
with st.sidebar:
    try:
        render_live_sidebar_panel()
    except Exception:
