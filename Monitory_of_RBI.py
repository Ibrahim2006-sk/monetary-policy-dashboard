...
Updated: Added Advanced Live Market section (yfinance) — placed before PDF export.
"""
...
# NEW: yfinance for live market data
...
# --------------------
# --------------------
# NEW SECTION: Advanced Live Market Data (yfinance)
# --------------------
# This section is intentionally self-contained and placed here so it does NOT change previous logic.
# It requires 'yfinance' installed in your environment.
# --------------------
st.subheader("Advanced Live Market Data (yfinance)")

# helper for plotly template (keep PLOTLY_TEMPLATE consistent)
def _get_plot_template():
    # keep same style as rest of app; if you want theme switching later, adapt here
    return PLOTLY_TEMPLATE if PLOTLY_TEMPLATE else ("plotly_white")

# 1) Indian Indexes (NIFTY, BANKNIFTY, FINNIFTY, India VIX)
st.markdown("### 📈 Indian Indexes")
index_map = {
    "NIFTY 50": "^NSEI",
    "BANKNIFTY": "^NSEBANK",
    "FINNIFTY": "^NSEFIN",
...
sel_index = st.selectbox("Select Index (yfinance)", list(index_map.keys()), index=0, key="adv_idx")
try:
    df_index = yf.download(index_map[sel_index], period="1y", interval="1d", progress=False, threads=False)
    if not df_index.empty:
        fig = px.line(df_index.reset_index(), x="Date", y="Close", title=f"{sel_index} — 1 Year", template=_get_plot_template())
        st.plotly_chart(fig, use_container_width=True)
        st.metric(f"{sel_index} (Last Close)", f"{df_index['Close'].iloc[-1]:.2f}")
    else:
        st.info(f"No yfinance series for {sel_index}")
except Exception as e:
    st.warning(f"yfinance failed for {sel_index}: {e}")

st.markdown("---")

# 2) Government bond yields (best-effort via yfinance tickers)
st.markdown("### 🇮🇳 Indian Govt Bond Yields (proxy tickers)")
bond_map = {
    "10Y G-Sec (proxy)": "^IR10Y",
    "5Y G-Sec (proxy)": "^IR5Y",
    "1Y G-Sec (proxy)": "^IR1Y"
}
sel_bond = st.selectbox("Select Bond (yfinance proxy)", list(bond_map.keys()), index=0, key="adv_bond")
try:
    df_bond = yf.download(bond_map[sel_bond], period="1y", interval="1d", progress=False, threads=False)
    if not df_bond.empty:
        figb = px.line(df_bond.reset_index(), x="Date", y="Close", title=f"{sel_bond} — 1 Year", template=_get_plot_template())
        st.plotly_chart(figb, use_container_width=True)
    else:
        st.info(f"No yfinance series for {sel_bond}")
except Exception as e:
    st.warning(f"yfinance failed for {sel_bond}: {e}")

st.markdown("---")

# 3) Forex INR crosses
st.markdown("### 💱 Forex: INR Crosses")
fx_map = {
    "USD/INR": "USDINR=X",
    "EUR/INR": "EURINR=X",
    "GBP/INR": "GBPINR=X",
...
sel_fx = st.selectbox("Select FX pair", list(fx_map.keys()), index=0, key="adv_fx")
try:
    df_fx = yf.download(fx_map[sel_fx], period="1y", interval="1d", progress=False, threads=False)
    if not df_fx.empty:
        figfx = px.line(df_fx.reset_index(), x="Date", y="Close", title=f"{sel_fx} — 1 Year", template=_get_plot_template())
        st.plotly_chart(figfx, use_container_width=True)
        st.metric(f"{sel_fx} (Last Close)", f"{df_fx['Close'].iloc[-1]:.4f}")
    else:
        st.info(f"No yfinance series for {sel_fx}")
except Exception as e:
    st.warning(f"yfinance failed for {sel_fx}: {e}")

st.markdown("---")

# 4) Commodities (Gold / Brent)
st.markdown("### 🪙 Commodities: Gold & Brent Oil")
comm_map = {
    "Gold (XAU/USD)": "GC=F",
    "Brent Oil": "BZ=F",
    "WTI Oil": "CL=F"
}
sel_comm = st.selectbox("Select commodity", list(comm_map.keys()), index=0, key="adv_comm")
try:
    df_comm = yf.download(comm_map[sel_comm], period="1y", interval="1d", progress=False, threads=False)
    if not df_comm.empty:
        figc = px.line(df_comm.reset_index(), x="Date", y="Close", title=f"{sel_comm} — 1 Year", template=_get_plot_template())
        st.plotly_chart(figc, use_container_width=True)
        st.metric(f"{sel_comm} (Last)", f"{df_comm['Close'].iloc[-1]:.2f}")
    else:
        st.info(f"No yfinance series for {sel_comm}")
except Exception as e:
    st.warning(f"yfinance failed for {sel_comm}: {e}")

st.markdown("---")

# 5) Sector heatmap (simple proxies)
st.markdown("### 🔥 Sector Heatmap — India (proxy stocks)")
sector_proxies = {
    "Banks": "HDFCBANK.NS",
    "IT": "INFY.NS",
    "Energy": "RELIANCE.NS",
...
for name, ticker in sector_proxies.items():
    try:
        df_sp = yf.download(ticker, period="5d", interval="1d", progress=False, threads=False)
        if not df_sp.empty and "Close" in df_sp.columns and len(df_sp) >= 2:
            pct = (df_sp["Close"].iloc[-1] - df_sp["Close"].iloc[0]) / df_sp["Close"].iloc[0] * 100
            sector_changes[name] = round(pct, 2)
    except Exception:
        sector_changes[name] = np.nan

if sector_changes:
    heat_df = pd.DataFrame.from_dict(sector_changes, orient="index", columns=["5d %"])
    try:
        fig_heat = px.imshow(heat_df.T, text_auto=True, aspect="auto", color_continuous_scale="RdYlGn", labels=dict(x="Sector", y="Metric"))
...
# 6) Crypto INR (BTC, ETH)
st.markdown("### ₿ Crypto (INR)")
crypto_map = {"BTC-INR": "BTC-INR", "ETH-INR": "ETH-INR"}
sel_crypto = st.selectbox("Select crypto pair", list(crypto_map.keys()), index=0, key="adv_crypto")
try:
    df_crypto = yf.download(crypto_map[sel_crypto], period="1y", interval="1d", progress=False, threads=False)
    if not df_crypto.empty:
        fig_cr = px.line(df_crypto.reset_index(), x="Date", y="Close", title=f"{sel_crypto} — 1 Year", template=_get_plot_template())
        st.plotly_chart(fig_cr, use_container_width=True)
        st.metric(f"{sel_crypto} (Last)", f"{df_crypto['Close'].iloc[-1]:.2f}")
    else:
        st.info("Crypto data unavailable via yfinance for this pair.")
except Exception as e:
    st.warning(f"yfinance failed for {sel_crypto}: {e}")

st.markdown("---")

# 7) Simple Market Sentiment (VIX-based) and summary metrics
st.markdown("### 📊 Market Sentiment Snapshot")
try:
    vix_df = yf.download("^INDIAVIX", period="1mo", interval="1d", progress=False, threads=False)
    if not vix_df.empty:
        latest_vix = vix_df["Close"].iloc[-1]
        sentiment = "Fear" if latest_vix > 16 else "Neutral" if latest_vix > 12 else "Greed"
        st.metric("India VIX", f"{latest_vix:.2f}")
        st.metric("Sentiment", sentiment)
    else:
        st.info("India VIX data not available via yfinance.")
except Exception:
    st.info("India VIX not available (yfinance).")

# Short summary metrics: NIFTY & USDINR if available
try:
    nifty = yf.download("^NSEI", period="5d", interval="1d", progress=False, threads=False)
    usdinr = yf.download("USDINR=X", period="5d", interval="1d", progress=False, threads=False)
    cols = st.columns(4)
    if not nifty.empty:
        cols[0].metric("NIFTY (Last)", f"{nifty['Close'].iloc[-1]:.2f}", delta=f"{nifty['Close'].iloc[-1]-nifty['Close'].iloc[0]:.2f}")
    if not usdinr.empty:
        cols[1].metric("USD/INR (Last)", f"{usdinr['Close'].iloc[-1]:.4f}", delta=f"{usdinr['Close'].iloc[-1]-usdinr['Close'].iloc[0]:.4f}")
except Exception:
    pass

st.markdown("Advanced market section complete. If yfinance fails for a ticker, check your environment's internet and yfinance installation.")
st.markdown("---")
