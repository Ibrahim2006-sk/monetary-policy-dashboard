# full_rbi_dashboard_with_theme_and_yfinance.py
"""
Monetary Policy Dashboard — RBI Theme (Light/Dark) + yfinance Live Market Tab (Option B)
Patched version: integrates theme switcher and uses get_plotly_template() for all charts.
Requires: rbi_yfinance_live_module.py in same folder (created previously).
"""
...
# yfinance live module (must be in same folder)
from rbi_yfinance_live_module import render_live_market_tab
...
# Header theme toggle (placed at top-right via columns)
# We'll render the header after applying CSS, but keep a small toggle control at top
# Create temporary placeholder for toggle in sidebar to avoid layout shift
with st.sidebar:
    st.write("")  # spacer
...
# --------------------
# HELPERS (original, unchanged logic)
# --------------------
def safe_num(x):
    try:
        return float(x)
...
# --------------------
# RBI LIVE scraping helpers (unchanged)
# --------------------
@st.cache_data
...
with rbi_live_tab:
    ...
    st.caption(
        "Note: RBI WSS pages sometimes change structure. If a block fails, you can still rely on uploaded CSVs in the Liquidity & Repo tab."
    )
...
# --------------------
# LIVE MARKET DATA TAB (from yfinance module)
# --------------------
